#!/usr/bin/env python3
"""Vater-Sohn-Prinzip (Grandfather-Father-Son) Rotation fuer BeeTown-Backups.
...
Aufruf: imkerei-backup-rotate.py <verzeichnis> <max_gesamt>
"""
import sys
import os
import re
...
NAME_RE = re.compile(r"^imkerei-backup-(\d{4})-(\d{2})-(\d{2})-(\d{6})\.tar\.gz$")
...
def main():
    if len(sys.argv) != 3:
        print("Aufruf: imkerei-backup-rotate.py <verzeichnis> <max_gesamt>", file=sys.stderr)
        return 1
